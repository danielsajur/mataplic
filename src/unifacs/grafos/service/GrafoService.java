...
import unifacs.grafos.models.Dijkstra;
...
public class GrafoService {
	...
	private List<Aresta> createArestas(List<String> strArestas,
		...
		for (String strAresta : strArestas) {
			...
			Vertice verticeEntrada = findVerticeById(strAresta.split("-")[0],
					vertices);
			Vertice verticeSaida = findVerticeById(strAresta.split("-")[1],
					vertices);
	...
	private void populateVerticesWithAresatas(final List<Vertice> vertices,
	...
	private boolean temArestaParalela(Aresta aresta, List<Aresta> arestas) {
		...
		if (contains) {
			System.out
					.println("A aresta já existe! Não é permito aresta paralela!");
		}
		return contains;
	}
