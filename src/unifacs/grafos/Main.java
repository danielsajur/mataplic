...
public class Main {
	...
	static {
		...
		MENU.add(" Algoritmo de Belmanford");
		MENU.add("S a i r");
	}
	...
	public static void main(String[] args) {
		...
		int itemMenu = 0;
		String idVerticeOrigem, idVerticeDestino;
		...
		while (itemMenu < MENU.size()) {
			...
			case 9:
				...
				break;
			case 10:
				System.out.print("\nInforme seu vertice de origem: ");
				idVerticeOrigem = scanner.next();
	...
	private static void printVerticesAdjacentes(final GrafoService service,
			final Grafo grafo) {

		Vertice verticeTestado = selecionarVertice(service, grafo, null);
		List<Vertice> verticesAdjacentes = service
				.getVerticesAdjacentes(verticeTestado);
		System.out
				.println("  Vertices adjacentes de " + verticeTestado.getId());
	...
	private static void adicionarAresta(final GrafoService service, Grafo grafo) {
		...
		Vertice verticeSaida = selecionarVertice(service, grafo,
				"Selecione o Vértice de saída: ");
		Vertice verticeEntrada = selecionarVertice(service, grafo,
				"Selecione o Vértice de entrada: ");
	...
	private static Aresta selecionarAresta(GrafoService service, Grafo grafo) {
		...
		for (Aresta aresta : grafo.getArestas()) {
			System.out.println("  " + (++indice) + " - Vertice "
					+ aresta.getId());
		}
	...
	private static void removerVertice(final GrafoService service,
			final Grafo grafo) {
	...
	@SuppressWarnings("resource")
	private static void adicionarVertice(final GrafoService service,
			final Grafo grafo) {
		Scanner scanner = new Scanner(System.in);
	...
	@SuppressWarnings("resource")
	private static Vertice selecionarVertice(GrafoService service, Grafo grafo,
			String texto) {
	...
	private static void ExcuteAlgoritmoDijkstra(GrafoService service,
			Grafo grafo, String origem, String destino) {
		service.ExcuteAlgoritmoDijkstra(grafo, origem, destino);
	}
}
